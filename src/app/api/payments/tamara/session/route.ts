...
import { tamaraConfigured } from "@/lib/payments/provider-config";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const { redirectUrl } = await startTamaraSession({
      userId: resolved.id,
      buyer: {
        email: user.email,
    ...
    const msg = err instanceof Error ? err.message : "tamara_failed";
    return NextResponse.json(
      { error: "Tamara checkout failed", code: process.env.NODE_ENV === "production" ? undefined : msg },
      { status: 502 },
    );
  }
}
